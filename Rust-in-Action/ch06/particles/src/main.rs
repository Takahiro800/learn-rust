...
// GlobalAllocトレイトを満足させるというマークをALLOCATORにつける
#[global_allocator]

// プログラムの実行中に行われたアロケーションについて、それぞれの所要時間をSTDOUTに出力する
...
impl World {
    ...
    fn remove_shpaes(&mut self, n: i32) {
        for _ in 0..n.abs() {
...
impl Particle {
    fn new(world: &World) -> Particle {
        ...
        Particle {
            height: 4.0,
            width: 4.0,
            position: [x, y].into(), // into()は[f64; 2]型の配列をVec2dに変換する
